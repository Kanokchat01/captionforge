...
understanding, 2-stage scene-report pipeline), optionally polishes +
self-critiques sarcastic/humor styles via Gemma (Fireworks, own
credentials) for the Gemma bonus prize, then writes /output/results.json.
Must exit 0, must finish within 10 minutes total, must never crash the
whole run because one clip failed, and must never let one stuck task blow
the whole container's time budget.
"""
import json
import os
import sys
...
def process_task(task: dict, gemini: GeminiCaptioner, gemma: GemmaAssistant) -> dict:
    ...
    if time_remaining() <= config.CRITICAL_TIME_THRESHOLD_SECONDS:
        print(f"[skip] task {task_id}: critical time remaining, using fallback without attempting Gemini")
    ...
    try:
        ...
        # this on the shared GeminiCaptioner instance — it's reused across
        # worker threads, so per-call state must stay local to this call.
...
def main():
    ...
    futures = {pool.submit(process_task, task, gemini, gemma): task for task in scheduled_tasks}
