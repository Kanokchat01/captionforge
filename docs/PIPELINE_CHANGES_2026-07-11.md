...
│                                                                │
│ 2. STAGE 1 — SCENE REPORT   [vision: kimi-k2p7-code]           │
...
| บทบาท | โมเดล | เหตุผล |
|---|---|---|
| Stage 1 vision | `kimi-k2p7-code` | scene report ละเอียด+ขุดมุกเก่งสุด, ไม่ hallucinate เทียบภาพจริง |
...
### `src/config.py`
- เปลี่ยน default โมเดล: vision `minimax-m3` → **`kimi-k2p7-code`**; text → **`glm-5p2`**;
